...
def deployContainer(image, env) {
  docker.image('lachlanevenson/k8s-kubectl:v1.5.2').inside {
    withCredentials([[$class: "FileBinding", credentialsId: 'KubeConfig', variable: 'KUBE_CONFIG']]) {
      def kubectl = "kubectl --kubeconfig=\$KUBE_CONFIG --certificate-authority=/home/ec2-user/kube-keys/ca.pem  --context=${env} --server=https://kz8s-apiserver-dev-375024213.us-west-2.elb.amazonaws.com"
